...
const gamesSlice = createSlice({
  ...
  reducers: {
    createGame: (
      state,
      action: PayloadAction<{ homeTeam: string; awayTeam: string; homeScore: number; awayScore: number }>
    ) => {
    ...
    updateGameScore: (state, action: PayloadAction<{ id: string; homeScore: number; awayScore: number }>) => {
    ...
    deleteFinishedGame: (state, action: PayloadAction<string>) => {
        const gameId = action.payload;
        const gameIndex = state.finishedGames.findIndex((game) => game.id === gameId);
        if (gameIndex !== -1) {
...
export const { createGame, updateGameScore, finishGame, deleteFinishedGame ,deleteGame} = gamesSlice.actions;
